...
while evalDate <= endDate:
    ...
    if to_dt_date(evalDate) in calibrered_params_ts.keys():
        continue
    print(evalDate)
    try:
    ...
    if df.loc[to_dt_date(evalDate),0] == 'SR709.CZC':
...
with open(os.path.abspath('..')+'/intermediate_data/svi_calibration_sr_calls.pickle','wb') as f:
    pickle.dump([calibrered_params_ts],f)

print('svi',svi_dataset)
with open(os.path.abspath('..')+'/intermediate_data/svi_dataset_sr_calls.pickle','wb') as f:
