...
def calulate_market_model_price_sse(
        spot, mktData, mktFlds, optionData, optionFlds, optionids,yield_ts,dividend_ts,flat_vol_ts,option_type):
    try:
        sse = 0.0
        for idx, optionid in enumerate(optionids):
            ...
            if optionData[optionFlds.index('call_or_put')][optionDataIdx] in option_type:
...
def calulate_market_model_price_sse_single_optiontype(
        spot, mktData, mktFlds, optionData, optionFlds, optionids,yield_ts,dividend_ts,flat_vol_ts,optiontype):
    try:
        sse = 0.0
        for idx, optionid in enumerate(optionids):
            optionDataIdx   = optionData[optionFlds.index('wind_code')].index(optionid)
            ...
            close           = mktData[mktFlds.index('close')][mktindex]
            if optionData[optionFlds.index('call_or_put')][optionDataIdx] == '认购':
                optiontype1 = ql.Option.Call
            else:
                optiontype1 = ql.Option.Put
            if optiontype != optiontype1: continue
            exercise = ql.EuropeanExercise(maturitydt)
            payoff   = ql.PlainVanillaPayoff(optiontype, strike)
            option   = ql.EuropeanOption(payoff, exercise)
            process  = ql.BlackScholesMertonProcess(ql.QuoteHandle(ql.SimpleQuote(spot)), dividend_ts, yield_ts,
...
def estimiate_bs_constant_vol_single_optiontype(evalDate, calendar, daycounter,optiontype):
    try:
        vols, spot, mktData, mktFlds, optionData, optionFlds, optionids = wind_data.get_wind_data(evalDate)
        ql.Settings.instance().evaluationDate = evalDate
        curve       = wind_data.get_curve_treasury_bond(evalDate, daycounter)
        ...
        while vol > step:
            flat_vol_ts = ql.BlackVolTermStructureHandle(ql.BlackConstantVol(evalDate, calendar, vol, daycounter))
            sse = calulate_market_model_price_sse_single_optiontype(
                spot, mktData, mktFlds, optionData, optionFlds, optionids, yield_ts, dividend_ts, flat_vol_ts,optiontype)
            if sse < min_sse:
                min_sse = sse
                estimate_vol = vol
