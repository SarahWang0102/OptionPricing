...
def wind_data_50etf_option_intraday(datestr):
    db_data = []
    datasource = 'wind'
    ...
    for (i2, row) in df_optionchain.iterrows():
        windcode = row['option_code']
        amt_strike = row['strike_price']
        contract_month = str(row['month'])[-4:]
        sec_name = row['option_name']
        if row['call_put'] == '认购' : cd_option_type = 'call'
        elif row['call_put'] == '认沽' : cd_option_type = 'put'
        if sec_name[-1] == 'A':
