...
with open(os.getcwd()+'/intermediate_data/hedging_daily_hedge_errors_svi_call.pickle','rb') as f:
...
print("SVI Model Average Hedging Percent Error, CALL : ")

print("="*100)
print("%20s %20s %30s" % ("contract month","moneyness", "avg hedging error(%)"))
print("-"*100)
for i in range(4):
    print("%20s %20s %25s" % (i,' < 0.97',round(sum(mny_0.get(i))*100/len(mny_0.get(i)),4)))
    print("%20s %20s %25s" % (i,' 0.97 - 1.00', round(sum(mny_1.get(i))*100 / len(mny_1.get(i)),4)))
    print("%20s %20s %25s" % (i,' 1.00 - 1.03', round(sum(mny_2.get(i))*100 / len(mny_2.get(i)),4)))
    print("%20s %20s %25s" % (i,' > 1.03', round(sum(mny_3.get(i))*100 / len(mny_3.get(i)),4)))
    print("-" * 100)
'''
...
for dt in svi_pct.keys():
    avg_svi_errors_M0 += sum(svi_pct.get(dt).get(0)[1])
    avg_bs_errors_M0 += sum(bs_pct.get(dt).get(0)[1])
    avg_svi_errors_M1 += sum(svi_pct.get(dt).get(1)[1])
    avg_bs_errors_M1 += sum(bs_pct.get(dt).get(1)[1])
    avg_svi_errors_M2 += sum(svi_pct.get(dt).get(2)[1])
    avg_bs_errors_M2 += sum(bs_pct.get(dt).get(2)[1])
    avg_svi_errors_M3 += sum(svi_pct.get(dt).get(3)[1])
    avg_bs_errors_M3 += sum(bs_pct.get(dt).get(3)[1])
