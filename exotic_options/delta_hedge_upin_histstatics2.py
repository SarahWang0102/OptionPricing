...
while begin_date < end_date:
    ...
    if init_svi <= 0.01 or init_bs <= 0.01 : continue
    ...
    if math.isnan(portfolio_net_svi/ init_svi) or math.isnan(portfolio_net_bs/ init_bs): continue
    avgholding_svi = sum(deltacont_svi) / len(deltacont_svi)
    avgholding_bs = sum(deltacont_bs) / len(deltacont_bs)
    ...
    holdings_svi.append(avgholding_svi)
    holdings_bs.append(avgholding_bs)
    ...
    print("%20s %20s %20s %20s %20s %20s" % (
        ...
        round(tradedvalue_svi / avgholding_svi, 4)))
print('=' * 200)
