...
#barrier = 2.615
#dk_cont = [-300.0,-200.0,-100.0,0.0,100.0,200.0,300.0]
#dk = -200.0
dk = 0.0
l_cont = [1.0,1.5,2.0,2.5,3.0]
...
for leverage in l_cont:
    ...
    while begin_date < begin_date2:
        ...
        intraday = pd.read_json(os.path.abspath('..') + '\marketdata\intraday_sr_' + datestr + '.json')
        ...
        pnl_svi = -tradingcost_svi
        pnl_bs = -tradingcost_bs
...
results.update({'barrier': leverages})
...
#print(df)
df.to_csv(os.path.abspath('..')+'/results/delta_hedge_sr_ame_ls.csv')