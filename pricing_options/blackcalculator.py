...
class blackcalculator:
    ...
    # 全Delta: dOption/dS = dOption/dS + dOption/dSigma * dSigma/dS
    # dSigma/dS = -(K/S)*(dSigma/dk)
    # dOption/dS = dOption/dS - vega * (K/S)*(dSigma/dk)
    def delta_total(self,spot,ttm,dSigma_dK):
        delta = self.delta(spot)
        return delta - self.vega(ttm) * (self.strike/spot)*dSigma_dK


    def vega(self,ttm):
        temp = math.log(self.strike / self.forward) / self.variance
        # actually DalphaDsigma / SQRT(T)
        DalphaDsigma = self.dAlpha_dD1 * (temp + 0.5)
        DbetaDsigma = self.dBeta_dD2 * (temp - 0.5)
        temp2 = DalphaDsigma * self.forward + DbetaDsigma * self.x
        return self.discount * math.sqrt(ttm) * temp2
