...
for barrier_pct in barrier_cont:
        ...
        while begDate < maturitydt:
            ...
            if not balanced:
                try:
                    ...
                    continue
                # rebalancing positions
                # if begDate == maturitydt: print(strike,daily_close,price_svi)
                tradingcost_svi, cash_svi, portfolio_net_svi, totalfees_svi, tradedamt_svi = \
                    exotic_util.calculate_hedging_positions(
    ...
    # print(df)
    df.to_csv(os.path.abspath('..') + '/results3/dh_'+barrier_type+
              '_r='+str(rebalancerate) + '_b=' + str(barrier_pct) + '_3.csv')
